...
def aws(jump_host,zookeeper_port,region,profile):
    instances=[]
...
@click.argument('jump_host')
@click.argument('zookeeper_urls')
@click.argument('zookeeper_ips')
@click.argument('schemaregistry_ips',default='')
@click.option('-zp','--zookeeper_port',default='2181')
@click.option('-sp','--schemaregistry_port',default='8081')
def manual(jump_host, zookeeper_urls, zookeeper_ips, schemaregistry_ips, zookeeper_port, schemaregistry_port):
    instances=[]
    click.echo(' * using manual ip\'s ...')
    man = ManualInstances()
    instances += man.getIps('zookeeper_urls',zookeeper_urls, zookeeper_port)
    ...
    zookeeper_ips_urls = list(zip(zookeeper_ips.split(','), zookeeper_urls.split(',')))

    print_instances(instances)
    add_local_interfaces(instances)
    connect_ssh_tunnel_manual(jump_host, zookeeper_ips_urls, zookeeper_port)
...
def connect_ssh_tunnel_manual(jump_host, zookeeper_ips_urls, zookeeper_port):
    click.echo(' * connecting to jump host ' + jump_host)
    opts = []
